        ...
        : XrmPage
    {
        ...
        private LoginResult Login(IWebDriver driver, Uri uri, SecureString username, SecureString password, Action<LoginRedirectEventArgs> redirectAction)
        {
            ...
            if (!online)
            {
                ...
                By idp = By.ClassName("idp");
                driver.WaitUntilAvailable(idp,
                    $"The Federation Login Page never displayed '{username}' could not be signed in.");

                if (driver.IsVisible(idp))
                {
                    try
                    {
                        IWebElement pofederationlink = driver.FindElement(idp);
                        pofederationlink.Click();
                    }
                    catch
                    {
                    }
                ...
                driver.FindElement(By.Id("userNameInput")).SendKeys(username.ToUnsecureString());
                driver.FindElement(By.Id("userNameInput")).SendKeys(Keys.Tab);

                driver.FindElement(By.Id("passwordInput")).SendKeys(password.ToUnsecureString());
                driver.FindElement(By.Id("passwordInput")).SendKeys(Keys.Tab);
